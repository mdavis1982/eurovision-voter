<x-app-layout title="Countries">
    <x-slot name="header">
        <div class="flex justify-between">
            ...
            <x-link-button :href="route('account.countries.create')">Add a Country</x-link-button>
        </div>
    </x-slot>
                                    ...
                                    <div class="px-4 py-4 sm:px-6">
                                        <div class="flex items-center justify-between">
                                            ...
                                            <div class="ml-2 flex-shrink-0 flex">
                                                <p class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $country->isCurrentlyVoting() ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                                    {{ $country->isCurrentlyVoting() ? 'Currently Voting' : 'Not Currently Voting' }}
                                                </p>
                                            </div>
                                        </div>
                                    </div>
            ...
            @else
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6 bg-white border-b border-gray-200 text-center text-gray-500">
                        There are no countries registered.
                    </div>
                </div>
            @endforelse
        </div>
